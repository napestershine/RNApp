...
import { retrieve, reset } from '../../actions/foodsymbol/show';
import { del, loading, error } from '../../actions/foodsymbol/delete';
import { list } from '../../actions/foodsymbol/list';
    ...
    return (
        <View style={styles.actionStyle}>
          <SocialIcon
              ...
              type='plus-circle'
              iconColor='#3faab4'
              onPress={() => Actions.foodsymbolCreate()}
          />
          <SocialIcon
              iconSize={34}
              type='edit'
              iconColor='#3faab4'
              onPress={() => Actions.foodsymbolUpdate({id})}
          />
          <SocialIcon
              iconSize={34}
              type='minus-circle'
  ...
  render() {
    ...
    return (
        <View style={ {flex: 1} }>
          <ScrollView>
            {item &&
            <Card title="FoodSymbol">
              <List title="title">
                {Show.renderRow('id', item['@id'])}
                {Show.renderRow('mark', item['mark'])}
                {Show.renderRow('symbol', item['symbol'])}
...
const mapStateToProps = (state) => {
  return {
    error: state.foodsymbol.show.error,
    loading: state.foodsymbol.show.loading,
    retrieved: state.foodsymbol.show.retrieved,
    deleteError: state.foodsymbol.del.error,
    deleteLoading: state.foodsymbol.del.loading,
    deleted: state.foodsymbol.del.deleted,
  };
};
