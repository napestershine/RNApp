...
import { retrieve, reset } from '../../actions/restaurantphoto/show';
import { del, loading, error } from '../../actions/restaurantphoto/delete';
import { list } from '../../actions/restaurantphoto/list';
    ...
    return (
        <View style={styles.actionStyle}>
          <SocialIcon
              ...
              type='plus-circle'
              iconColor='#3faab4'
              onPress={() => Actions.restaurantphotoCreate()}
          />
          <SocialIcon
              iconSize={34}
              type='edit'
              iconColor='#3faab4'
              onPress={() => Actions.restaurantphotoUpdate({id})}
          />
          <SocialIcon
              iconSize={34}
              type='minus-circle'
  ...
  render() {
    ...
    return (
        <View style={ {flex: 1} }>
          <ScrollView>
            {item &&
            <Card title="RestaurantPhoto">
              <List title="title">
                {Show.renderRow('id', item['@id'])}
                {Show.renderRow('path', item['path'])}
                {Show.renderRow('bigThumbnailPath', item['bigThumbnailPath'])}
                {Show.renderRow('smallThumbnailPath', item['smallThumbnailPath'])}
...
const mapStateToProps = (state) => {
  return {
    error: state.restaurantphoto.show.error,
    loading: state.restaurantphoto.show.loading,
    retrieved: state.restaurantphoto.show.retrieved,
    deleteError: state.restaurantphoto.del.error,
    deleteLoading: state.restaurantphoto.del.loading,
    deleted: state.restaurantphoto.del.deleted,
  };
};
