...
import { retrieve, reset } from '../../actions/temprestaurant/show';
import { del, loading, error } from '../../actions/temprestaurant/delete';
import { list } from '../../actions/temprestaurant/list';
    ...
    return (
        <View style={styles.actionStyle}>
          <SocialIcon
              ...
              type='plus-circle'
              iconColor='#3faab4'
              onPress={() => Actions.temprestaurantCreate()}
          />
          <SocialIcon
              iconSize={34}
              type='edit'
              iconColor='#3faab4'
              onPress={() => Actions.temprestaurantUpdate({id})}
          />
          <SocialIcon
              iconSize={34}
              type='minus-circle'
...
const mapStateToProps = (state) => {
  return {
    error: state.temprestaurant.show.error,
    loading: state.temprestaurant.show.loading,
    retrieved: state.temprestaurant.show.retrieved,
    deleteError: state.temprestaurant.del.error,
    deleteLoading: state.temprestaurant.del.loading,
    deleted: state.temprestaurant.del.deleted,
  };
};
