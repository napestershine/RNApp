...
import { retrieve, reset } from '../../actions/restaurantordercount/show';
import { del, loading, error } from '../../actions/restaurantordercount/delete';
import { list } from '../../actions/restaurantordercount/list';
    ...
    return (
        <View style={styles.actionStyle}>
          <SocialIcon
              ...
              type='plus-circle'
              iconColor='#3faab4'
              onPress={() => Actions.restaurantordercountCreate()}
          />
          <SocialIcon
              iconSize={34}
              type='edit'
              iconColor='#3faab4'
              onPress={() => Actions.restaurantordercountUpdate({id})}
          />
          <SocialIcon
              iconSize={34}
              type='minus-circle'
  ...
  render() {
    ...
    return (
        <View style={ {flex: 1} }>
          <ScrollView>
            {item &&
            <Card title="RestaurantOrderCount">
              <List title="title">
...
const mapStateToProps = (state) => {
  return {
    error: state.restaurantordercount.show.error,
    loading: state.restaurantordercount.show.loading,
    retrieved: state.restaurantordercount.show.retrieved,
    deleteError: state.restaurantordercount.del.error,
    deleteLoading: state.restaurantordercount.del.loading,
