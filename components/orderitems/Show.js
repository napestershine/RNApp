...
import { retrieve, reset } from '../../actions/orderitems/show';
import { del, loading, error } from '../../actions/orderitems/delete';
import { list } from '../../actions/orderitems/list';
    ...
    return (
        <View style={styles.actionStyle}>
          <SocialIcon
              ...
              type='plus-circle'
              iconColor='#3faab4'
              onPress={() => Actions.orderitemsCreate()}
          />
          <SocialIcon
              iconSize={34}
              type='edit'
              iconColor='#3faab4'
              onPress={() => Actions.orderitemsUpdate({id})}
          />
          <SocialIcon
              iconSize={34}
              type='minus-circle'
  ...
  render() {
    ...
    return (
        <View style={ {flex: 1} }>
          <ScrollView>
            {item &&
            <Card title="OrderItems">
              <List title="title">
                {Show.renderRow('id', item['@id'])}
                {Show.renderRow('quantity', item['quantity'])}
                {Show.renderRow('price', item['price'])}
                {Show.renderRow('comment', item['comment'])}
...
const mapStateToProps = (state) => {
  return {
    error: state.orderitems.show.error,
    loading: state.orderitems.show.loading,
    retrieved: state.orderitems.show.retrieved,
    deleteError: state.orderitems.del.error,
    deleteLoading: state.orderitems.del.loading,
    deleted: state.orderitems.del.deleted,
  };
};
