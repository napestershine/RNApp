...
import { retrieve, reset } from '../../actions/foodtopping/show';
import { del, loading, error } from '../../actions/foodtopping/delete';
import { list } from '../../actions/foodtopping/list';
    ...
    return (
        <View style={styles.actionStyle}>
          <SocialIcon
              ...
              type='plus-circle'
              iconColor='#3faab4'
              onPress={() => Actions.foodtoppingCreate()}
          />
          <SocialIcon
              iconSize={34}
              type='edit'
              iconColor='#3faab4'
              onPress={() => Actions.foodtoppingUpdate({id})}
          />
          <SocialIcon
              iconSize={34}
              type='minus-circle'
...
const mapStateToProps = (state) => {
  return {
    error: state.foodtopping.show.error,
    loading: state.foodtopping.show.loading,
    retrieved: state.foodtopping.show.retrieved,
    deleteError: state.foodtopping.del.error,
    deleteLoading: state.foodtopping.del.loading,
    deleted: state.foodtopping.del.deleted,
  };
};
