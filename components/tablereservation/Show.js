...
import { retrieve, reset } from '../../actions/tablereservation/show';
import { del, loading, error } from '../../actions/tablereservation/delete';
import { list } from '../../actions/tablereservation/list';
    ...
    return (
        <View style={styles.actionStyle}>
          <SocialIcon
              ...
              type='plus-circle'
              iconColor='#3faab4'
              onPress={() => Actions.tablereservationCreate()}
          />
          <SocialIcon
              iconSize={34}
              type='edit'
              iconColor='#3faab4'
              onPress={() => Actions.tablereservationUpdate({id})}
          />
          <SocialIcon
              iconSize={34}
              type='minus-circle'
  ...
  render() {
    ...
    return (
        <View style={ {flex: 1} }>
          <ScrollView>
            {item &&
            <Card title="TableReservation">
              <List title="title">
                {Show.renderRow('id', item['@id'])}
                {Show.renderRow('tableNumber', item['tableNumber'])}
...
const mapStateToProps = (state) => {
  return {
    error: state.tablereservation.show.error,
    loading: state.tablereservation.show.loading,
    retrieved: state.tablereservation.show.retrieved,
    deleteError: state.tablereservation.del.error,
    deleteLoading: state.tablereservation.del.loading,
