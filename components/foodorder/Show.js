...
import { retrieve, reset } from '../../actions/foodorder/show';
import { del, loading, error } from '../../actions/foodorder/delete';
import { list } from '../../actions/foodorder/list';
    ...
    return (
        <View style={styles.actionStyle}>
          <SocialIcon
              ...
              type='plus-circle'
              iconColor='#3faab4'
              onPress={() => Actions.foodorderCreate()}
          />
          <SocialIcon
              iconSize={34}
              type='edit'
              iconColor='#3faab4'
              onPress={() => Actions.foodorderUpdate({id})}
          />
          <SocialIcon
              iconSize={34}
              type='minus-circle'
  ...
  render() {
    ...
    return (
        <View style={ {flex: 1} }>
          <ScrollView>
            {item &&
            <Card title="FoodOrder">
              <List title="title">
                {Show.renderRow('id', item['@id'])}
                {Show.renderRow('amount', item['amount'])}
                {Show.renderRow('serviceTax', item['serviceTax'])}
                {Show.renderRow('otherTax', item['otherTax'])}
                {Show.renderRow('deliveryCost', item['deliveryCost'])}
                {Show.renderRow('discount', item['discount'])}
                {Show.renderRow('total', item['total'])}
                {Show.renderRow('orderType', item['orderType'])}
...
const mapStateToProps = (state) => {
  return {
    error: state.foodorder.show.error,
    loading: state.foodorder.show.loading,
    retrieved: state.foodorder.show.retrieved,
    deleteError: state.foodorder.del.error,
    deleteLoading: state.foodorder.del.loading,
    deleted: state.foodorder.del.deleted,
  };
};
