...
import { del, loading, error } from '../../actions/restaurant/delete';
import { list } from '../../actions/restaurant/list';
    ...
    return (
        <View style={styles.actionStyle}>
          <SocialIcon
              ...
              type='plus-circle'
              iconColor='#3faab4'
              onPress={() => Actions.restaurantCreate()}
          />
          <SocialIcon
              iconSize={34}
              type='edit'
              iconColor='#3faab4'
              onPress={() => Actions.restaurantUpdate({id})}
          />
          <SocialIcon
              iconSize={34}
              type='minus-circle'
          ...
          <ScrollView>
            ...
            <Card title="Restaurant">
              <List title="title">
                ...
                {Show.renderRow('minimumOrderValue', item['minimumOrderValue'])}
                {Show.renderRow('deliveryTime', item['deliveryTime'])}
                {Show.renderRow('logo', item['logo'])}
                {Show.renderRow('logoImage', item['logoImage'])}
                {Show.renderRow('status', item['status'])}
                {Show.renderRow('backgroundImage', item['backgroundImage'])}
                {Show.renderRow('backgroundImageFile', item['backgroundImageFile'])}
                {Show.renderRow('profileLogoFile', item['profileLogoFile'])}
                {Show.renderRow('profileLogo', item['profileLogo'])}
                {Show.renderRow('haveDelivery', item['haveDelivery'])}
                {Show.renderRow('haveTakeAway', item['haveTakeAway'])}
                {Show.renderRow('haveFreeFood', item['haveFreeFood'])}
                {Show.renderRow('slug', item['slug'])}
                {Show.renderRow('email', item['email'])}
                {Show.renderRow('phone', item['phone'])}
                {Show.renderRow('website', item['website'])}
                {Show.renderRow('about', item['about'])}
                {Show.renderRow('isFeatured', item['isFeatured'])}
                {Show.renderRow('restaurantWorkSchedules', item['restaurantWorkSchedules'])}
