...
import { del, loading, error } from '../../actions/country/delete';
import { list } from '../../actions/country/list';
    ...
    return (
        <View style={styles.actionStyle}>
          <SocialIcon
              ...
              type='plus-circle'
              iconColor='#3faab4'
              onPress={() => Actions.countryCreate()}
          />
          <SocialIcon
              iconSize={34}
              type='edit'
              iconColor='#3faab4'
              onPress={() => Actions.countryUpdate({id})}
          />
          <SocialIcon
              iconSize={34}
              type='minus-circle'
          ...
          <ScrollView>
            ...
            <Card title="Country">
              <List title="title">
                ...
                {Show.renderRow('capital', item['capital'])}
                {Show.renderRow('citizenship', item['citizenship'])}
                {Show.renderRow('countryCode', item['countryCode'])}
                {Show.renderRow('currency', item['currency'])}
                {Show.renderRow('currencyCode', item['currencyCode'])}
                {Show.renderRow('currencySubUnit', item['currencySubUnit'])}
                {Show.renderRow('fullName', item['fullName'])}
                {Show.renderRow('iso3', item['iso3'])}
                {Show.renderRow('iso2', item['iso2'])}
                {Show.renderRow('name', item['name'])}
                {Show.renderRow('regionCode', item['regionCode'])}
                {Show.renderRow('subRegionCode', item['subRegionCode'])}
                {Show.renderRow('eea', item['eea'])}
