...
import { retrieve, reset } from '../../actions/verificationcode/show';
import { del, loading, error } from '../../actions/verificationcode/delete';
import { list } from '../../actions/verificationcode/list';
    ...
    return (
        <View style={styles.actionStyle}>
          <SocialIcon
              ...
              type='plus-circle'
              iconColor='#3faab4'
              onPress={() => Actions.verificationcodeCreate()}
          />
          <SocialIcon
              iconSize={34}
              type='edit'
              iconColor='#3faab4'
              onPress={() => Actions.verificationcodeUpdate({id})}
          />
          <SocialIcon
              iconSize={34}
              type='minus-circle'
  ...
  render() {
    ...
    return (
        <View style={ {flex: 1} }>
          <ScrollView>
            {item &&
            <Card title="VerificationCode">
              <List title="title">
                {Show.renderRow('id', item['@id'])}
                {Show.renderRow('userId', item['userId'])}
                {Show.renderRow('channel', item['channel'])}
                {Show.renderRow('phone', item['phone'])}
                {Show.renderRow('code', item['code'])}
                {Show.renderRow('verifyingTimes', item['verifyingTimes'])}
...
const mapStateToProps = (state) => {
  return {
    error: state.verificationcode.show.error,
    loading: state.verificationcode.show.loading,
    retrieved: state.verificationcode.show.retrieved,
    deleteError: state.verificationcode.del.error,
    deleteLoading: state.verificationcode.del.loading,
    deleted: state.verificationcode.del.deleted,
  };
};
