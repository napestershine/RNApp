...
import { retrieve, reset } from '../../actions/restaurantearningcount/show';
import { del, loading, error } from '../../actions/restaurantearningcount/delete';
import { list } from '../../actions/restaurantearningcount/list';
    ...
    return (
        <View style={styles.actionStyle}>
          <SocialIcon
              ...
              type='plus-circle'
              iconColor='#3faab4'
              onPress={() => Actions.restaurantearningcountCreate()}
          />
          <SocialIcon
              iconSize={34}
              type='edit'
              iconColor='#3faab4'
              onPress={() => Actions.restaurantearningcountUpdate({id})}
          />
          <SocialIcon
              iconSize={34}
              type='minus-circle'
  ...
  render() {
    ...
    return (
        <View style={ {flex: 1} }>
          <ScrollView>
            {item &&
            <Card title="RestaurantEarningCount">
              <List title="title">
...
const mapStateToProps = (state) => {
  return {
    error: state.restaurantearningcount.show.error,
    loading: state.restaurantearningcount.show.loading,
    retrieved: state.restaurantearningcount.show.retrieved,
    deleteError: state.restaurantearningcount.del.error,
    deleteLoading: state.restaurantearningcount.del.loading,
